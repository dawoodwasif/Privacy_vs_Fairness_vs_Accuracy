...
from main_qfed_nsmc_fair import predict_and_evaluate as predict_smc
...
def evaluate_mia_at_thresholds(thresholds, dataset_train, dataset_test, dict_users, args):
    mia_results = {}
    for threshold in thresholds:
        print(f"\nEvaluating MIA at threshold: {threshold}")
        args.threshold = threshold  # Set the polynomial modulus degree

        # Train and get predictions from the shadow model for HE
        shadow_model_he = train_shadow_model(dataset_train, dataset_test, dict_users, args, predict_smc)
        ...
        mia_results[threshold] = success_rate
...
if __name__ == '__main__':
    ...
    mia_results = evaluate_mia_at_thresholds(thresholds, dataset_train, dataset_test, dict_users, args)

    # Print out the MIA results for each polynomial modulus degree
    print("\nMIA Success Rates at Different SMC Threshold:")
    for threshold, success_rate in mia_results.items():
        print(f"SMC Threshold {threshold}: MIA Success Rate = {success_rate:.2f}")
