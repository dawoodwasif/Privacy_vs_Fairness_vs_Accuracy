...
def args_parser():
    ...
    # federated arguments
    parser.add_argument('--q', type=float, default=1.0, help="q-FedAvg q value")
    ...
    # DP parameters
    parser.add_argument('--epsilon', type=float, default=32.0, help="epsilon value for differential privacy")
    parser.add_argument('--delta', type=float, default=1e-5, help="delta value for differential privacy") # fixed: 10 -5
    parser.add_argument('--sensitivity', type=float, default=1.0, help="sensitivity of the query for differential privacy") # fixed: 1
    parser.add_argument('--dp_mechanism', type=str, default='laplace', choices=['laplace', 'gaussian', 'randomized_response', 'exponential'], help="Differential privacy mechanism to use")

    return parser.parse_args()

def add_dp_noise(gradient, epsilon, delta, sensitivity, mechanism):
    """Adds noise to the gradient based on DP parameters."""
    if mechanism == 'laplace':
        noise_scale = sensitivity / epsilon
        noise = torch.distributions.Laplace(0, noise_scale).sample(gradient.size())
    elif mechanism == 'gaussian':
        noise_scale = sensitivity * np.sqrt(2 * np.log(1.25 / delta)) / epsilon
        noise = torch.normal(0, noise_scale, size=gradient.size())
    elif mechanism == 'randomized_response':
        # Assuming gradient is binary (0/1) for simplicity
        noise = torch.bernoulli(torch.tensor([0.5]))  # Coin flip for randomized response
        noise = (2 * noise - 1) * epsilon
    elif mechanism == 'exponential':
        # Exponential mechanism for non-numeric queries, here applied as a noise generator
        score_function = gradient.norm(p=1)  # Example score function
        noise = torch.exp(epsilon * score_function / (2 * sensitivity))
        noise = torch.distributions.Laplace(0, noise).sample(gradient.size())
    else:
...
def predict_and_evaluate(args, net_glob, dataset_train, dataset_test, dict_users):
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            ...
            # Apply DP noise to each layer of the model
            for key in w.keys():
                w[key] = add_dp_noise(w[key], args.epsilon, args.delta, args.sensitivity, args.dp_mechanism)
...
if __name__ == '__main__':
    ...
    custom_name = 'q-fedavg_DP_{}'.format(args.q)
    weights_dir = os.path.join('./weights', '{}_{}_{}_{}_ep={}_weights'.format(args.dataset, args.model, args.epochs, custom_name, args.epsilon))
