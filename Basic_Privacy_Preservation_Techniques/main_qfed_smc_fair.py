...
class AESEncryptor:
    ...
    def encrypt(self, data):
        ...
        encrypted_data = {k: cipher.encrypt(pad(pickle.dumps(v.cpu().numpy()), AES.block_size)) for k, v in data.items()}
        return encrypted_data

    def decrypt(self, data):
        cipher = AES.new(self.key, AES.MODE_ECB)
        decrypted_data = {k: torch.tensor(pickle.loads(unpad(cipher.decrypt(v), AES.block_size))) for k, v in data.items()}
...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            ...
            encrypted_w = encryptor.encrypt(w)
        ...
        # Decrypt encrypted weights before aggregation
        decrypted_locals = [encryptor.decrypt(w) for w in w_locals]
        
        # Aggregate decrypted weights using q-FedAvg
        w_glob = copy.deepcopy(decrypted_locals[0])
        for lk in w_glob.keys():
            w_glob[lk] = sum(h_k * w[lk] for w, h_k in zip(decrypted_locals, h_locals)) / sum(h_locals)
