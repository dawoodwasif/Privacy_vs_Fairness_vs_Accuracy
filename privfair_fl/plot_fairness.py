...
"./log_vehicle/qffedavg_samp2_run1_q10_20_test.csv"
]

# accuracies = [ 
#     "./log_fmnist_old/qffedavg_samp2_run1_q0_3010_test.csv",
#     "./log_fmnist_old/qffedavg_samp2_run1_q0_4500_test.csv",
#     "./log_fmnist_old/qffedavg_samp2_run1_q0_6000_test.csv",
# ]
...
def get_bar_y(filename, num_clients, num_bar=39):
    ...
    euclidean_distances = np.zeros(num_clients)
    ...
    # Calculate Euclidean distance from the mean for each client
    for i in range(num_clients):
        euclidean_distances[i] = np.linalg.norm(accuracies[0][i] - mean_accuracies)

    avg_euclidean_distance = np.mean(euclidean_distances)
    var_euclidean_distance = np.var(euclidean_distances)
    ...
    print("file: {}\n, worst 10: {}, std: {}; best 10: {}, std: {}; variance: {}, std: {}; Euclidean distance variance: {}".format(\
        filename, avg_b, std_b, avg_g, std_g, avg_var, std_var, var_euclidean_distance))

    print("############################################")

    return mean_clients, std_clients, mean_accuracies, var_euclidean_distance
...
mean_y1, std_y1, mean_accu1, var_euclidean1 = get_bar_y(baseline, num_clients[0])
mean_y2, std_y2, mean_accu2, var_euclidean2 = get_bar_y(method, num_clients[0])
mean_y3, std_y3, mean_accu3, var_euclidean3 = get_bar_y(uniform, num_clients[0])
