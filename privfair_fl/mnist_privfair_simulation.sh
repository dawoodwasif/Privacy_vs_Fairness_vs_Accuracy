...
# Define q values
q_values1=(1 5 10 15 20)    # For the first task
q_values2=(4 8)         # For the second task

# Define epsilon values for DP (LDP and GDP)
epsilon_values1=(0.5 1) # For task 1
# epsilon_values2=(0.5 1 1.5 2 2.5 3 3.5 4 4.5 5 5.5 6 6.5 7 7.5 8 8.5 9 9.5 10) # For task 2
epsilon_values2=(2 4 8 16 32) # For task 2


# Define modulus values for HE
modulus_values1=(8192 8192)  # For task 1
...
run_simulation() {
    ...
    local q=$5
    local lr=$6
    local base_dir="./mnist_privfair_experiments_LAST/${optimizer}/${privacy_tech}_${param_value}_q${q}_lr${lr}"
...
for q in "${q_values1[@]}"; do
    for epsilon in "${epsilon_values1[@]}"; do
        for optimizer in "${optimizers[@]}"; do
            # Run LDP simulation
            run_simulation "$optimizer" "LDP" "$epsilon" "mnist" $q $lr_value &
            ((current_jobs++))
            ...
            # Run GDP simulation
            run_simulation "$optimizer" "GDP" "$epsilon" "mnist" $q $lr_value &
            ((current_jobs++))
            if [[ $current_jobs -ge $max_jobs ]]; then
                wait -n
                ((current_jobs--))
    ...
    for modulus in "${modulus_values1[@]}"; do
        for optimizer in "${optimizers[@]}"; do
            run_simulation "$optimizer" "HE" "$modulus" "mnist" $q $lr_value &
            ((current_jobs++))
            if [[ $current_jobs -ge $max_jobs ]]; then
                wait -n
                ((current_jobs--))
