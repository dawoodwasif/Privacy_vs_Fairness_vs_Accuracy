...
run_simulation() {
    ...
    local base_dir="./mnist_pp_sota_fl_experiments_epsilon=1/${optimizer}/${privacy_technique}_q${reweighting_factor}_lr${lr}"
...
for optimizer in "${optimizers[@]}"; do
    for privacy_technique in "${privacy_techniques[@]}"; do
        run_simulation "$optimizer" "$privacy_technique" "mnist" "$q_value" "$learning_rate"
    done
done
