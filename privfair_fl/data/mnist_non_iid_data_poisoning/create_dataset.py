...
def apply_data_poisoning(y_train, poison_rate=0.1):
    """Flip a subset of labels to simulate data poisoning."""
    num_samples = int(len(y_train) * poison_rate)
    poisoned_indices = random.sample(range(len(y_train)), num_samples)
    
    for idx in poisoned_indices:
        original_label = y_train[idx]
        new_label = (original_label + random.randint(1, 9)) % 10  # Flip to a different label
...
def main():
    ...
    train_output = "./data/train/mytrain_non_iid_poisoned.json"
    test_output = "./data/test/mytest_non_iid_poisoned.json"
    ...
    for user in range(NUM_USER):
        ...
        # Apply data poisoning to a subset of each user's training labels
        y_user_train[user] = apply_data_poisoning(y_user_train[user], poison_rate=0.1)
