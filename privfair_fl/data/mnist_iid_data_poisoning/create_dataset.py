...
def apply_data_poisoning(y_train, poison_rate=0.1):
    """
    Apply data poisoning by flipping a certain percentage of training labels.
    
    Args:
    - y_train: List of training labels.
    - poison_rate: Fraction of samples to poison (0.1 = 10%).
    ...
    for idx in poisoned_indices:
        # Flip the label to a random incorrect label
        original_label = y_train[idx]
        new_label = (original_label + random.randint(1, 9)) % 10
...
def main():
    ...
    train_output = "./data/train/mytrain_poisoned.json"
    test_output = "./data/test/mytest_poisoned.json"
    ...
    y_train = apply_data_poisoning(y_train, poison_rate=0.1)  # Apply data poisoning to training labels
