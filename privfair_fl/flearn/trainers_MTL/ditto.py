        ...
        for i in range(self.num_rounds + 1):
            ...
            for idx in indices:
                ...
                for _ in range(self.local_iters):
                    ...
                    try:
                        data_batch = next(batches[c])  # Try to fetch the next batch
                    except StopIteration:
                        # Reset the generator when StopIteration is raised
                        batches[c] = gen_batch(c.train_data, self.batch_size, self.num_rounds * self.local_iters)
                        data_batch = next(batches[c])  # Fetch next batch after resetting
