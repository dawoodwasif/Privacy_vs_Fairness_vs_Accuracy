...
# Define epsilon values for DP (for both LDP and GDP)
epsilon_values=(0.5 1 1.5 2.5 3 3.5 4 4.5 5 5.5 6 6.5 7 7.5 8 8.5 9 9.5 10)

# Define modulus values for HE
modulus_values=(4096 8192 16384 32768)
...
run_simulation() {
    ...
    local q=$5
    local lr=$6
    local base_dir="./mnist_privfair_experiments/${optimizer}/${privacy_tech}_${param_value}_q${q}_lr${lr}"
...
# Run simulations for LDP and GDP with different epsilon values
for epsilon in "${epsilon_values[@]}"; do
    for optimizer in "${optimizers[@]}"; do
        # Run LDP simulation
        run_simulation "$optimizer" "LDP" "$epsilon" "mnist" $q_value $lr_value &
        ((current_jobs++))
        ...
        # Run GDP simulation
        run_simulation "$optimizer" "GDP" "$epsilon" "mnist" $q_value $lr_value &
        ((current_jobs++))
        if [[ $current_jobs -ge $max_jobs ]]; then
            wait -n
            ((current_jobs--))
...
# Run simulations for HE with different modulus values
for modulus in "${modulus_values[@]}"; do
    for optimizer in "${optimizers[@]}"; do
        run_simulation "$optimizer" "HE" "$modulus" "mnist" $q_value $lr_value &
        ((current_jobs++))
        if [[ $current_jobs -ge $max_jobs ]]; then
            wait -n
            ((current_jobs--))
